...
class Giveaway extends Rooms.SimpleRoomGame {
	...
	static getSprite(set: PokemonSet): [ID, Chat.VNode] {
		...
		return [
			species.id,
			<img src="http://play.pokemonshowdown.com/data/pokemon-showdown/server/chat-plugins/{`/sprites/ani${shiny}/${spriteid}.gif`}" />,
		];
	}
		...
		for (const species of Dex.species.all()) {
			...
			if (res) {
				...
				return <>
					...
					<a href="http://play.pokemonshowdown.com/data/pokemon-showdown/server/chat-plugins/{`http://www.serebii.net/pokedex-sm/location/${num}.shtml`}">
...
function makePageHeader(user: User, pageid?: string) {
	...
	for (const i in titles) {
		...
		if (pageid === i) {
			...
			urls.push(<>{icon} <a href="http://play.pokemonshowdown.com/data/pokemon-showdown/server/chat-plugins/{`/view-giveaways-${i}`}" target="replace">{title}</a></>);
		}
	}
	buf.push(<>{[urls]}</>, <hr />);
...
function formatFakeButton(url: string, text: Chat.VNode): Chat.VNode {
	return <a class="button" style={{textDecoration: 'inherit'}} target="replace" href="http://play.pokemonshowdown.com/data/pokemon-showdown/server/chat-plugins/{url}">{text}</a>;
}
